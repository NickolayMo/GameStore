...
{
    public class GameStoreSeedData
    {
        private EfDbContext _context;
        private UserManager<AdminUser> _manager;
        public GameStoreSeedData(EfDbContext context, UserManager<AdminUser> manager)
        {
            _context = context;
            _manager = manager;
        }
        ...
        {
            if (await _manager.FindByNameAsync("Admin") == null)
            {
                ...
                IdentityResult s = await _manager.CreateAsync(adminUser, "Kolya@456");
            }

            if (!await _context.Games.AnyAsync())
            {

                string nativeSQLQuery = "INSERT INTO Game(Name, Description, Category, Price) " +
"VALUES " +
...
"	('Dead Space 3', 'В Dead Space 3 Айзек Кларк и суровый солдат Джон Карвер отправляются в космическое путешествие, чтобы узнать о происхождении некроморфов.', 'Шутер', 499.00);";
                _context.Database.ExecuteSqlCommand(nativeSQLQuery);
